...
CXR_ReFusE is a [TODO: quick blurb here]. We remove all references to priors from the training dataset ([TODO: give this a name and link to PhysioNet]) to prevent there being hallucinated references to non-existent priors in CXR_ReFusE's generated reports. See our [papers]() [here]()!
...
Download the `data` folder [here](https://drive.google.com/file/d/1VGfJ8aH9B9GwdNMDPqrYzayk0pN-6e0a/view?usp=sharing) and place it in the `CXR_ReFusE` directory.
...
To finetune the visual entailment (VE) model, run:
...
This script relies on `ve_train.json`, which is created from `mimic_train.json` and `mimic_train_impressions_chexbert.tsv` using `generate_ve_file.py`, as well as `ve_dev.json` and `ve_test.json` (all included in the `data` folder linked above).

Linked here are the VE model checkpoints [with](https://drive.google.com/file/d/1UYyUcfzMqt6BWLbVNPsASuUpJFoLieeB/view?usp=sharing) and [without](https://drive.google.com/file/d/11cZ2DSqS52p9jH42ok7BbbUiWUEaF_RC/view?usp=sharing) removing references to priors from the MIMIC-CXR reports corpus.
...
## Citing
If you are making use of this repository, please cite the following papers:
